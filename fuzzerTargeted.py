...
def get_bytes(filename):
	...
	while counter < num_of_flips:
		...
		for x in chosen_indexes:
			...
			picked_target = random.choice(indexes)
		...
		if new_number[picked_target] == "1":
			new_number[picked_target] = "0"
		else:
			new_number[picked_target] = "1"
...
def exif(counter,data):
	try:
		img = iio.imread('muta/mutated'+str(counter)+'.jpg')
	except (IOError, SyntaxError,OSError,ValueError) as e:
		f = open('muta/mutated.txt', "a")
